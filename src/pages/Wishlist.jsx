...
export default function Wishlist({ onNavigate, onViewProduct }) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] ambient-mesh pb-16 pt-4">
      <div className="max-w-[1480px] mx-auto px-4 md:px-margin">
        {/* Breadcrumbs */}
        <div className="flex items-center gap-2 text-xs font-medium text-neutral-500 mb-4 px-3 py-1 rounded-full glass-pill w-fit">
          <button
        ...
        {/* Header Title Bar */}
        <div className="glass-panel rounded-3xl p-5 md:p-6 shadow-soft border border-white/80 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-5 md:gap-6">
            {wishlistedProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="glass-card rounded-2xl sm:rounded-3xl p-3 sm:p-4 transition-all duration-300 flex flex-col justify-between group relative active:scale-[0.98]"
                >
                  ...
                  <div
                    ...
                    className="relative aspect-square w-full rounded-2xl overflow-hidden bg-white/70 mb-3 cursor-pointer flex items-center justify-center p-2 border border-black/[0.03]"
                  >
                    <img
                      src={product.image}
                      alt={product.name}
