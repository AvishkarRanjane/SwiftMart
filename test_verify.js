...
Object.entries(counts).forEach(([cat, count]) => {
  let expected = 10;
  if (cat === "electronics") expected = 12;
  else if (cat === "furniture") expected = 16;
  else if (cat === "wishlist" || cat === "top-value" || cat === "gadgets-appliances" || cat === "skincare-haircare" || cat === "fashion-deals" || cat === "popular-nearby") expected = 15;
  else if (cat === "grocery") expected = 16;
  else if (cat === "dmart-packs") expected = 12;
  
  console.log(` - ${cat}: ${count} items (Expected: ${expected})`);
  if (count !== expected) {
    console.error(
      `❌ Category ${cat} does not have exactly ${expected} items! Found: ${count}`,
    );
    process.exit(1);
  }
});
    ...
    p.variants.forEach((v) => {
      if (v.image) {
        ...
        if (!fs.existsSync(vImgPath)) {
          console.warn(
            `⚠️ Variant image missing: ${v.image} for product ${p.id}`,
          );
          missingImages++;
        }
      }
      if (v.images) {
        v.images.forEach((vImg) => {
          ...
          if (!fs.existsSync(vPath)) {
            console.warn(
              `⚠️ Variant gallery image missing: ${vImg} for product ${p.id} (${v.name})`,
            );
            missingImages++;
          }
        });
